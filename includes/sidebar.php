...
$navItems = [
    ['label' => 'Dashboard', 'href' => ($basePath ?: '') . '/index.php', 'icon' => '▦', 'active' => true],
    ['label' => 'Vehicles', 'href' => '#', 'icon' => '▱'],
    ['label' => 'Vehicle Log Book', 'href' => '#', 'icon' => '▤'],
    ['label' => 'Drivers', 'href' => '#', 'icon' => '◌'],
    ['label' => 'Maintenance', 'href' => '#', 'icon' => '◇'],
    ['label' => 'Reports', 'href' => '#', 'icon' => '▧'],
    ['label' => 'Pre-Inspection Report', 'href' => '#', 'icon' => '☷'],
    ['label' => 'Post-Inspection Report', 'href' => '#', 'icon' => '☑'],
    ['label' => 'Service Providers', 'href' => '#', 'icon' => '▥'],
    ['label' => 'Communications', 'href' => '#', 'icon' => '□'],
    ['label' => 'Comm. History', 'href' => '#', 'icon' => '◷'],
    ['label' => 'Estates & Works', 'href' => '#', 'icon' => '▔'],
];
?>
<aside id="app-sidebar" class="fixed inset-y-0 left-0 z-30 flex w-64 -translate-x-full flex-col bg-fleet-sidebar text-fleet-sidebar-text shadow-xl transition-transform duration-200 lg:translate-x-0">
    ...
    <nav class="flex-1 space-y-1 px-2">
        <?php foreach ($navItems as $item): ?>
            <a href="<?= htmlspecialchars($item['href'], ENT_QUOTES, 'UTF-8'); ?>" class="flex h-10 items-center gap-3 rounded-lg px-4 text-sm font-semibold transition <?= !empty($item['active']) ? 'bg-fleet-sidebar-active text-fleet-warning' : 'text-fleet-sidebar-text hover:bg-fleet-sidebar-soft hover:text-white'; ?>">
                <span class="flex w-5 justify-center text-base"><?= htmlspecialchars($item['icon'], ENT_QUOTES, 'UTF-8'); ?></span>
